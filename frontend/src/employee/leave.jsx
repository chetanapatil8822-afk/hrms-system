...
export default function LeaveApplicationPage() {
  ...
  const fetchLeaveManagementData = async () => {
    ...
    try {
      // 🕵️‍♂️ Dynamic Role Extraction Strategy from JWT Payload
      if (token) {
        try {
  ...
  // Calculate total days between chosen date strings dynamically
  const calculateDays = (start, end) => {
    if (!start || !end) return 0;
    const sDate = new Date(start);
    const eDate = new Date(end);

    // Safety check against inverted date ranges selection bugs
  ...
  // 🚀 POST: DISPATCH COMPLIANT PAYLOAD DOWN TO PIPELINE ENDPOINT
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-white font-sans">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Casual Leave', used: balances.casual?.used || 0, max: balances.casual?.max || 8 },
            { label: 'Medical Leave', used: balances.medical?.used || 0, max: balances.medical?.max || 12 },
            { label: 'Earned Leave', used: balances.earned?.used || 0, max: balances.earned?.max || 15 },
            { label: 'Paid Leave', used: balances.paid?.used || 0, max: balances.paid?.max || 10 },
          ].map((item, idx) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col justify-between">
              ...
              <div className="overflow-x-auto">
                {leaveHistory.length === 0 ? (
                  <div className="text-center py-10 text-[10px] font-black uppercase text-gray-400 tracking-wider">
                    No historic leave records found on this account ledger.
                  </div>
                ) : (
            ...
            <div className="text-[10px] text-gray-400 font-black uppercase tracking-widest text-center pt-6 border-t border-gray-100 mt-4">
              Requests are routed for verified administrative review
            </div>
          </div>
